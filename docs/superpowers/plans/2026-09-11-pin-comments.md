...
- [x] Write and run failing real DB tests first: persistence, version/project
...
- [x] Implement all methods with persisted `authorize`, project/member locks,
  ...
  Composite FKs bind version/thread to the same workspace/project. Author FK
  binds the workspace member; live project membership is required at write time
  so historical authors remain valid after membership revocation.
...
- [x] Run DB suite sequentially with existing persistence private env. Never edit
  migrations 0000/0001 or move/init databases. Independent spec then quality review.

## HTTP and browser

- [x] Write browser regression: upload actual generated PNG → pin at 25%/40% →
...
- [x] Add session-authorized `/api/comments` GET/POST and
...
- [x] Extract current private-image UI into `src/app/review-canvas.tsx` which
...
- [x] Include explicit “Add pin” toggle, click image to select location, and
...
- [x] Verify unit/typecheck/full DB/build and Chrome journeys; inspect screenshots
