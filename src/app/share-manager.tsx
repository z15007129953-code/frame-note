...
export default function ShareManager({ presentationId, busy, request, execute }: Props) {
  ...
  return <section className="share-manager" aria-label="Share presentation">
    ...
    <p className="note">Anyone with the link can view all current and future versions in this presentation. Guest comments are optional; editing and uploads are never shared. Local preview links work only on this computer.</p>
    ...
    <label className="share-comments-option"><input type="checkbox" checked={allowComments} disabled={busy || loading} onChange={e => setAllowComments(e.target.checked)} /> Allow guest comments</label>
    ...
    {loading ? <p role="status">Loading share links…</p> : <ul className="share-list">
      {links.map((link, i) => <li key={link.id}>
        ...
        {link.revoked ? <span>Revoked</span> : <><span>{link.allowComments ? "Comments on" : "Read only"}</span><button className="quiet" disabled={busy} onClick={() => action(async () => {
