...
export async function migrate(connection: Connection, migrations?: readonly Migration[]): Promise<void> {
  const entries = migrations ?? await Promise.all(['0000_review.sql', '0001_demo_sessions.sql', '0002_comments.sql', '0003_shares.sql', '0004_guest_comments.sql'].map(async name => ({
